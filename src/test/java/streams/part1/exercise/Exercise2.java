...
class Exercise2 {
    ...
    void findEmployeeWithMaximumDurationAtOnePosition() {
        ...
        Employee expected = employees.stream()
                .max(Comparator.comparingInt(value -> value.getJobHistory().stream()
    ...
    void calcTotalSalaryWithCoefficientWorkExperience() {
        List<Employee> employees = getEmployees();
        final double BASIC_SALARY = 75_000;

        Double expected = employees.stream()
                .map(Employee::getJobHistory)
                .mapToDouble(value -> value.get(value.size() - 1).getDuration() < 3 ? BASIC_SALARY : BASIC_SALARY * 1.2)
                .sum();
